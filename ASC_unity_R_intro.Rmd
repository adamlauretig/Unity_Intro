...
If you are using PuTTy on a windows, the `[source filepath]` must use "\\" or "/" to separate and you will be asked for your username and password on ASC domain. This way is not recommended, a more convenient way of uploading files from local machine to Unity server is to use SHTP/SFTP connection at domain `unity.asc.ohio-state.edu` (with your username and password on ASC domain) via software like [FileZilla](https://filezilla-project.org/). As far as I tried, `pscp` is not supported.
...
## Existing problems
The current configuration of unity cluster does not allow local R library but ask you to install all packages to personal library. This causes a lot of problems, I would like to point out two of them.
