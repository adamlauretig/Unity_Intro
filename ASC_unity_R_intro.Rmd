...
/usr/local/R/3.3.2/bin/Rscript ~/toy_function.R
```
...
Under `#COMMANDS TO RUN`, `source /opt/intel/bin/compilervars.sh intel64` calls the necessary intel compiler, and `/usr/local/R/3.3.2/bin/Rscript` is the path for the `Rscript` command, to batch execute the script.  Just using `module load intel R` in the bash script causes the cluster to throw esoteric errors.  Either one can learn a great deal about unix file structures to solve this problem, or copy and paste the above code.  I recommend the second approach.
...
```
where the parallel function supplies the `mclapply` function, for which, we tell it to use three cores, in `mc.cores`.  The file path for loading and saving is just `~/`, since we'll put everything in the same directory.
...
Which will email us when the job starts, and ends. If it works, `toy_dt.rdata` with appear when we type `ls`, and an output file with the job name and number will appear as well.
